...
export const HomeScreen = () => {
	...
	useEffect(
		() => {
			fetch('/meme/').then(res => res.json()).then(
				(result) => {
					setIsLoaded(true);
					console.log(result);
					setPostsAMostrar(result.obj);
				}, (error) => {
					setIsLoaded(false);
				}
			)
			let filterArray = postsAMostrar.filter((post) => {
	...
	if (!isLoaded) {
		return (<div className="loadingthing"><p>Loading</p></div>);
	} else {
		return (
